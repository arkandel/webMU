...
<head>
	...
	<script>
		...
		function updateRoomList(roomDenizens)
		{
			document.getElementById('roomList').innerHTML = roomDenizens;
		}
		...
		$(document).ready(function() {
			log('Connecting...');
			Server = new FancyWebSocket('ws://192.168.1.165:9300');
			...
			Server.bind('message', function( payload ) {
				if (isJSON(payload)) // This is a server command. Do not show to the player.
				{
					var serverUpdate = JSON.parse(payload);
					switch(serverUpdate[0])
					{
						case 'updateRoomList':
							updateRoomList(serverUpdate[1]);
						break;
					}
				}
				else
